...
import { PDFDocument, rgb, StandardFonts } from 'pdf-lib';
import './modifyPdf.css'
import ModifyPdfViewer from '../modifyPdfViewer/modifyPdfViewer';
...
const ModifyPdf = ({ pdfBytes }) => {
  ...
  const handleModifyPdf = async () => {
    ...
    const textWidth = helveticaFont.widthOfTextAtSize(text, parseInt(fontSize));
    ...
    firstPage.drawText(textChange, {
      x: parseInt(xdist)+5,     //x:450 + 5
      y: 770-parseInt(ydist)+5,     //y:660 + 5
      size: parseInt(fontChange),
      font: timesNewRomanFont,
      color: rgb(0,0,0),
    });
    ...
    // firstPage.drawText(textChange, {
    //   x: parseInt(xdist),
    //   y: firstPage.getHeight() - parseInt(ydist),
    //   size: parseInt(fontChange),
    //   font: timesNewRomanFont,
    //   color: rgb(0, 0, 0),
    // });
  ...
  return (
    <div className="inp-div">
      ...
      <h4>
        ...
        <input className="inps" value={textChange} type='text' onChange={(e) => setTextChange(e.target.value)} />
      </h4>
      <h4>
        Enter x: <input className="inps" type='number' value={xdist} onChange={(e) => setXdist(e.target.value)} />
      </h4>
      <h4>
        Enter y: <input className="inps" type='number' value={ydist} onChange={(e) => setYdist(e.target.value)} />
      </h4>
      <h4>
        Enter Font size: <input value={fontChange} className="inps" type='number' onChange={(e) => setFontChange(e.target.value)} />
      </h4>
      <button className='modify-btn' onClick={handleModifyPdf}>UPDATE PDF</button>
      
      {modifiedPdfBytes && (
        <>
          <ModifyPdfViewer modifiedPdfBytes={modifiedPdfBytes} />
